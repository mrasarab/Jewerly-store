...
const addSameProductToCart = async (req, res) => {
  ...
  try {
    const checkBasketQuery = "SELECT * FROM carts WHERE email = ? AND itemId = ?";
    const [basketResults] = await conn.promise().query(checkBasketQuery, [email, itemId]);

    if (basketResults.length === 0) {
      return res.status(200).json({ message: "This product is not in the basket." });
    }

    const currentQuantity = basketResults[0].quantity;
    const newQuantity = currentQuantity + 1;

    const updateQuery = "UPDATE carts SET quantity = ? WHERE email = ? AND itemId = ?";

    await conn.promise().query(updateQuery, [newQuantity, email, itemId]);
    
    return res.status(200).json({ message: `Quantity of the product is now: ${newQuantity}` });
  } catch (error) {
...
module.exports = addSameProductToCart;
